...
export default async function EditPlanningEntryPage({
  params,
}: {
  ...
  if (!entry) redirect('/admin/overblik')
  ...
  async function deleteEntry() {
    'use server'
    const supabase = await createClient()
    await supabase.from('planning_entries').delete().eq('id', id)
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <section className="rounded-xl p-6 space-y-5" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        <form action={updateEntry} className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                defaultValue={entry.placement_comment ?? ''}
      ...
      <section className="rounded-xl p-5" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        <h2 className="text-sm font-semibold mb-2" style={{ color: 'var(--foreground)' }}>Slet placering</h2>
        <p className="text-xs mb-4" style={{ color: 'var(--muted)' }}>
          Dette sletter rækken permanent fra overblikket.
        </p>
        <form action={deleteEntry}>
          <button
            type="submit"
            className="px-4 py-2 rounded-lg text-sm font-semibold"
            style={{ background: 'var(--surface-2)', color: '#ef4444', border: '1px solid #ef444433' }}
          >
            Slet placering
          </button>
        </form>
